...
def get_funding_rate_kraken(symbol):
    exchange = ccxt.krakenfutures({'enableRateLimit': True})
    try:
        data = exchange.fetch_funding_rate(pair_for_kraken(symbol) + ":USD")
    except Exception:
        return None
    fr = float(data['fundingRate']) * 100
    interval_hours = 8
...
if __name__ == "__main__":
    ...
    for pair in pairs:
        ...
        kraken = get_funding_rate_kraken(pair)
        if not mexc or not kraken:
            continue

        # Déterminer quelle position long/short pour delta neutre optimal
        if mexc['funding_rate_percent'] < kraken['funding_rate_percent']:
            long_pos, short_pos = mexc, kraken
        else:
            long_pos, short_pos = kraken, mexc
